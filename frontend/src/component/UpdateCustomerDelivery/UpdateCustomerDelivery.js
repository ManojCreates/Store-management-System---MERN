...
function UpdateCustomerDelivery() {
    ...
    const fetchHandler = useCallback(async () => {
        await axios
            .get(`http://localhost:5001/customerDeliveries/${id}`)
            .then((res) => res.data)
            .then((data) => {
                setInputs({
                    ...data.customerDeliveries,
                    glassType: data.customerDeliveries.glassType || [],
                });
            });
    }, [id]);
    ...
    const handleSubmit = (e) => {
        ...
        if (isNameValid && isContactNumberValid && isEmailValid && isAddressValid) {
            sendRequest().then(() => history("/customerdeliverydetails"));
        }
    };

    const sendRequest = async () => {
        const fullContactNumber = `${selectedCountryCode}${inputs.contactNumber}`;
        await axios.put(`http://localhost:5001/customerDeliveries/${id}`, {
    ...
    return (
        <div>
            <h1>Update Customer Delivery</h1>
            <form onSubmit={handleSubmit}>
                ...
                <br /><br />
                <textarea name="specialInstructions" onChange={handleChange} value={inputs.specialInstructions} rows="5" cols="40" />
                <br /><br />
...
export default UpdateCustomerDelivery;
